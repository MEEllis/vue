    ...
    <div class="rank-info-list">
      <mt-cell v-for="(item,index) in songList" :title="item.filename" @click.native="playAudio(index)" :key="index">
        <span class="rank-index"
              :class="{'rank-list-good' : index<3, 'rank-list-first' : index==0, 'rank-list-second' : index==1, 'rank-list-third' : index==2}">{{index + 1}}
        </span>
  ...
  export default{
    ...
    methods: {
      ...
      getList(){
        ...
        this.$http.get('https://bird.ioliu.cn/v2?url=' + `http://m.kugou.com/rank/info/?rankid=${infoID}&page=1&json=true`).then(({data}) => {
