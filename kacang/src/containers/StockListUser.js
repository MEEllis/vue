...
import StockAccount from '../components/StockListUser/js/StockListUser'
// import { actionsStart } from '../actions/getPasswordCardList'
...
const stateItemSelector = (state) => state.getPasswordCardList.data || []
...
const stateSelector = createSelector (
    ...
    (stateItem) => {
        ...
        for(let k in itemState.Data){
            ...
            itemState.Data[k].key = parseInt(k)+parseInt(stateItem.PageSize)*(parseInt(stateItem.PageNumber)-1);
        }
        return itemState
    }
)
...
const mapStateToProps = (state, ownProps) => {
    return {
        pwdCardListData: stateSelector(state),
        isfetching: !!state.getPasswordCardList.isfetching || !!state.batchUpdatePrepaidCardIsUse.isfetching || !!state.batchRemovePrepaidAccount.isfetching,
...
const mapDispatchToProps = (dispatch, ownProps) => {
    return {
        getPasswordCardList: bindActionCreators(actionsStart, dispatch),
