 ...
 */
$("#add").click(function(){
	window.location.href="/manager/cw/test/repriceAdd/-999";
});
...
function loadmodal()
		...
		LoadTableUrl: "/manage/erp/fx/reprice",
...
//		inputbox: ".inputbox" //搜索条件框存放容器。遵照css选择器书写
		...
		$("#datetimepickerNo").datetimepicker({
			...
			var colNames = ['','序号','ID','调价部门','调价日期','调价人','调价总额','客户名称','备注','新增人',
			                '新增时间','修改人','修改时间','过账人','过账时间','红冲人','红冲时间'];
			var JqGridColModel=[
			                    {name:'',index:'', width:100,align:'center',formatter: addAndUpdate },
			                    {name:'index',index:'index', width:100,align:'center' ,sorttype:'int'},
								{name:'',index:'', width:100,align:'center', sorttype:'int',hidden:true},
								{name:'sName',index:'sName', width:100,align:'center', sorttype:'int'},
								{name:'billsDate',index:'billsDate', width:100,align:'center', sorttype:'string'},
								{name:'managersName',index:'managersName', width:100,align:'center', sorttype:'string'},
								{name:'bilisAmount',index:'bilisAmount', width:100,align:'center', sorttype:'string'},
								{name:'cName',index:'cName', width:100,align:'center', sorttype:'string'},
								{name:'remark',index:'remark', width:100,align:'center', sorttype:'string'},
								{name:'createName',index:'createName', width:100,align:'center', sorttype:'string'},
								{name:'createDate',index:'createDate', width:100,align:'center', sorttype:'string'},
								{name:'updateName',index:'updateName', width:100,align:'center', sorttype:'string'},	
								{name:'updateDate',index:'updateDate', width:100,align:'center', sorttype:'string'}, 
								{name:'postName',index:'postName', width:100,align:'center', sorttype:'string'},
								{name:'postDate',index:'postDate', width:100,align:'center', sorttype:'date'},
								{name:'invalidName',index:'invalidName', width:100,align:'center', sorttype:'string'},
								{name:'invalidDate',index:'invalidDate', width:100,align:'center', sorttype:'String'},
			                ];
...
//			})
			...
			$(document).on('click', '.glyphicon-pencil',function(e){
				...
				var rowId = $(this).parent().data('id');
				if(thisTitle == "修改"){
					window.location.href="/manager/cw/test/repriceAdd/"+rowId;
				}
			});
...
function addAndUpdate(cellvalue, options, rowObjec){
	var addAndDel = '<div class="operating" data-id="' + options.rowId + '"><span style="margin-right:0.7rem" id="remove_row" title="修改" class="glyphicon glyphicon-pencil"</span></div>';
	return addAndDel;
}
