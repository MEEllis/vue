...
function getLodop(oOBJECT, oEMBED) {
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            if ((LODOP == null) || (typeof(LODOP.VERSION) == "undefined")) {
                if (navigator.userAgent.indexOf('Chrome') >= 0)
                    $.zxsaas_plus.showalert("提示", erpHtmChrome)
                if (navigator.userAgent.indexOf('Firefox') >= 0)
                    $.zxsaas_plus.showalert("提示", erpHtmFireFox)
                if (is64IE) $.zxsaas_plus.showalert("提示", erpHtm64_Install); else if (isIE) $.zxsaas_plus.showalert("提示", erpHtmInstall); else
                    $.zxsaas_plus.showalert("提示", erpHtmInstall)
                return LODOP;
            }
            ;
        }
        ...
        if (LODOP.VERSION < "6.2.1.7") {
            if (needCLodop())
                $.zxsaas_plus.showalert("提示", erpCLodopUpdate); else if (is64IE) $.zxsaas_plus.showalert("提示", erpHtm64_Update); else if (isIE) $.zxsaas_plus.showalert("提示", erpHtmUpdate); else
                $.zxsaas_plus.showalert("提示", erpHtmUpdate)
            return LODOP;
        }
        ;
...
function getLodopObj(oOBJECT, oEMBED) {
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            if ((LODOP == null) || (typeof(LODOP.VERSION) == "undefined")) {
                if (navigator.userAgent.indexOf('Chrome') >= 0)
                    retObj.retMes=erpHtmChrome
                if (navigator.userAgent.indexOf('Firefox') >= 0)
                    retObj.retMes=erpHtmFireFox
                if (is64IE) {
