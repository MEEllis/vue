...
function modalsaveOrUpdate(num){
	...
	if(num == 1){
		...
		$('#myModalLabel').html('售后维修项目新增');
		$('.butHide').show();
	}else if(num == 2){
		//修改
		$('#myModalLabel').html('售后维修项目修改');
		$('.butHide').hide();
		//判断选中
...
function saveAndAddInpayClass(){
	...
	initialObj.saveAndCloseAprojectName(param,'auth_add',function(data){
...
function saveAndCloseTotherstorageClass(){
	...
	initialObj.saveAndCloseAprojectName(param,ac_jc,function(data){
		if($.ac_jc(data)) return false;  //判断权限
...
function delAprojectName(){
	...
	if(ids.length == 0){
		...
		$.zxsaas_plus.showconfirm("","是否确定删除选中数据?",function(){
			...
			initialObj.delAprojectName(delIds,function(data){
				if($.ac_jc(data)) return false;  //判断权限
...
function queryInpayClass(){
	...
	if(param.queryText){
		param.queryText=$.trim(param.queryText);
		$("#jqGrid_blocMessage").jqGrid('setGridParam',{ 
	        url:basePath + "/jxc/authority/AprojectName/findAprojectName",
	        postData:param, //发送数据 
	        page:1 
...
function loadmodal()
		...
		LoadTableUrl: basePath + "/jxc/authority/AprojectName/findAprojectName",
		...
		$("#datetimepickerStart").datetimepicker({
	      ...
	      todayButton:false    //关闭选择今天按钮
			...
			var colNames = ['公司名称','项目编码','项目名称','预设价格','备注','新增人','新增时间','修改人','修改时间','id','集团id','公司id','编码','新增人'];
			var JqGridColModel=[
								...
								{name:'predictPrice',index:'predictPrice',align:'center',sortable:false},
