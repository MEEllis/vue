...
function enable(){
	...
	$.zxsaas_plus.showconfirm("提示信息",'是否停用此科目吗？',function(){
		subjectAjax.enableFcompanySubjectTemplate(rowId,'/cw/fsubject/enableFgroupSubjectTemplate/',function(data){
			data == 1 ? $.zxsaas_plus.showalert("提示信息","停用科目成功!") : $.zxsaas_plus.showalert("提示信息","停用科目失败!");
			searchData();
		});
...
function deleteSubject(){
	...
	$.zxsaas_plus.showconfirm("提示信息",'是否删除此科目吗？',function(){
		subjectAjax.deleteFcompanySubjectTemplate(rowId,'/cw/fsubject/deleteFgroupSubjectTemplate/',function(data){
			data == 1 ? $.zxsaas_plus.showalert("提示信息","删除科目完成!") : $.zxsaas_plus.showalert("提示信息","删除科目失败!");
			searchData();
		});
	},function(){
...
function save(int){
	...
	var flag=true;
	if(formParams.subjectCode==""||formParams.subjectCode==null){
		$.zxsaas_plus.showalert("提示","科目编码必填！");
		flag=false;
	}else if(formParams.subjectName==""||formParams.subjectName==null){
		$.zxsaas_plus.showalert("提示","科目名称必填！");
		flag=false;
	}else if(formParams.subjectClssify==""||formParams.subjectClssify==null){
		$.zxsaas_plus.showalert("提示","科目类型数据带出必填！");
		flag=false;
	}else if(formParams.creditDirection==""||formParams.creditDirection==null){
		$.zxsaas_plus.showalert("提示","余额方向数据带出必填！");
		flag=false;
	}
	if(flag){
		if($('#subjectLabel').html() == '新增科目'){
			subjectAjax.saveFcompanySubjectTemplate(formParams,$('#currentAccountingYear').val(),'/cw/fsubject/saveFgroupSubjectTemplate/',function(data){
				data == 1 ? $.zxsaas_plus.showalert("信息提示","新增科目成功!") : $.zxsaas_plus.showalert("信息提示","新增科目失败!");
				searchData();
			});
		}else{
			subjectAjax.updateFcompanySubjectTemplate(formParams,'/cw/fsubject/updateFgroupSubjectTemplate',function(data){
				data == 1 ? $.zxsaas_plus.showalert("信息提示","修改科目成功!") : $.zxsaas_plus.showalert("信息提示","修改科目失败!");
				searchData();
			});
		}
...
function updateSubject(){
	...
	//根据id查询科目信息
	subjectAjax.getById(ids[0],'/cw/fsubject/findSubjectById/',function(data){
...
$(document).on('blur','.subjectCode',function(e){
	subjectAjax.blurSubjectCode($('#currentAccountingYear').val(),$('#subjectCode').val().trim(),'/cw/fsubject/blurSubjectCode',function(data){
		var obj = data.data.entity;
