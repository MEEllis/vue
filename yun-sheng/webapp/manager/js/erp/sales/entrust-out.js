...
function pageAjax(param){
	$.request({
		url:"../entrustOut/selectList/"+pageIndex,
		type : 'GET',  
		dataType: "json",
		data:param,
...
function fillPageData(row){
	...
	if(row.salesOutstorExNumList.length>0){
		$.each(row.salesOutstorExNumList,function(i,value){
			if(value.salesOutstorExImDraftList!=null && value.salesOutstorExImDraftList.length>0){
				 value.salesOutstorExImDraftList=JSON.stringify(value.salesOutstorExImDraftList);
			 }
			 if(isDraft==1){
...
function initDataGrid(){
	...
	var paras = {
	    ...
	    colNames:['仓库id','仓库名称','类别','商品编码','商品名称','品牌','型号','颜色','现存量','数量','单价','金额','商品备注','商品ID','ID','是否串号管理','串号列表'], 
	    colModel:
	    	[ 
             {name : 'storageId',index : 'storageId',align:'center',sortable: false,hidden: true}, 
             {name : 'storageName' ,sortable: false,index : 'storageName',width:'90px',fixed:true,align:'center',
            	 editable:true,
            	 edittype:'custom',
             ...
             {name : 'goodsName',sortable: false,index : 'goodsName',align:'center',edittype:'custom_bt_input',custom_element_bt_click:"showGoodsModal",editable:true}, 
             ...
             {name : 'stockNum',index : 'stockNum',align:'center',editable:false,sortable: false,formatter:'integer'},
             {name : 'goodsNumber',index : 'goodsNumber',align:'center',editable:true,sortable: false,formatter:'integer',editoptions:{onblur:"checkInput.checkNumPure(this,12)"}},
             {name : 'price',index : 'price',align:'center',editable:true,editrules:{number:true},formatter:'number',sortable: false,editoptions:{readonly:false,onfocus:"checkLine()",onblur:"gridCellOnBlur()",onblur:"checkInput.checkNum(this,12)"}},
             {name : 'amount',index : 'amount',align:'center',editable:false,editrules:{number:true},formatter:'number',sortable: false},
             {name : 'remark',index : 'remark',align:'center',hidden: false,editable:true,sortable: false,editoptions:{readonly:false,onblur:"gridCellOnBlur()"}},
             ...
             {name : 'salesOutstorExImDraftList',index : 'salesOutstorExImDraftList',editable:true,sortable: false,hidden: true},
           ]
...
function checkLine(){
	var storageName=$('#dataGrid').getCol('storageName');
	var goodsName=$('#dataGrid').getCol('goodsName');
	...
	if(storageName==''){
		...
		$(this).blur()
		$.zxsaas_plus.showalert("提示","请先选择商品");
	}
}
...
function delBtClick(){
	...
	$.zxsaas_plus.showconfirm("提示","是否确定删除此单据?",function(){
		...
		$.request({
			url:'/manager/entrustOut/delete/'+id,
			type:'GET',
			dataType:'json',
...
function saveBtnClick(){
	...
	if(isDraft==0){
		url="../salesOrder/updateRemark/wtck_update";
		ajaxType="GET";
...
function getSaveParam(){
	...
	if(ids.length>0){
		var salesOutstorExNumDraftList=new Array();
		$.each(ids,function(i,value){
			...
			if(rowData.goodsNumber!=0){
				...
				if(rowData.salesOutstorExImDraftList=="" || rowData.salesOutstorExImDraftList==null || rowData.salesOutstorExImDraftList==undefined){
					delete rowData["salesOutstorExImDraftList"];
				}else{
					if(! jQuery.isArray(rowData.salesOutstorExImDraftList)){
						rowData.salesOutstorExImDraftList=JSON.parse(rowData.salesOutstorExImDraftList);
					}
				}
				deleteRelateGoodsColumn(rowData);
				salesOutstorExNumDraftList.push(rowData);
			}
		});
		
		if(salesOutstorExNumDraftList.length>0){
			param.salesOutstorExNumDraftList=salesOutstorExNumDraftList;
		}else{
...
function changeStorckNum(obj){
	...
	if(currRow.goodsId!=undefined && storageId!=undefined){
