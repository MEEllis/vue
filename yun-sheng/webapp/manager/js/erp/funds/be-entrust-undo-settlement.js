...
function print(){
	...
	function todo(){
		$.printBills(basePath + '/funds/print/beEntrustUndoSettlement', 
			{
		      billsId:id,
...
function queryPage(){
	...
	//后台查询
	$.request({
		url: this.basePath + '/IcashMain/page/stcj_query',
		type : "post",
		dataType : 'json',
		data:model,
...
function initPageData(bills){
	...
	var formObj = bills;
	var mingxiList = bills.cashBeentrustDeldtlList;
...
function saveBtClick(){
	...
	//添加一些代码
	for (var int = 0; int < model.cashBeentrustDeldtlList.length; int++) {
		var deta  = model.cashBeentrustDeldtlList[int];
	...
	//添加一些代码
	//code......
	billsDao.saveBeEntrustUndoSettlementBills(model, function(data){
		if(data.result == 1){
			$.MsgBox('提示','操作成功');
...
function getPageData(){
	...
	//明细
	formObj.cashBeentrustDeldtlList = dataGrid.getGridDataList();
					...
					if(data.data.dataList.length != 0){
						for(var i=0;i<data.data.dataList.length;i++){
							ulHtml += '<li onclick = "imeiUlLiClick({'+
					          ...
					          'seteNum:\''+(data.data.dataList[i].instroNum-data.data.dataList[i].noSeteNum)+'\','+
...
function selectBeentrustStockReferenceOpen(){
	...
	callBack = function(){
		...
		for ( var i = 0; i < models.length; i++) {
			var model = models[i];
			model.remark = "";
			model.seteNum = $.parseInt(model.instroNum) - $.parseInt(model.noSeteNum);
//			model.seteNum = 1;
			model.setePrice = model.setePrice;
			model.refMainId = model.billsMainId;
			model.dissetePrice = model.setePrice;
			model.disseteNum = $.parseInt(model.instroNum) - $.parseInt(model.noSeteNum);
...
function initDataGrid(){
	...
	var paras = {
	    ...
	    colNames:['id','入库日期','商品ID','商品名称','入库数量','结算量','串号','辅助串号','结算价','本次撤结量','撤结单价','撤结总额','撤结后剩余数量', '备注', '备注'], 
	    colModel:
	    	[ {name : 'id',index : 'id',align:'left',sortable: false,hidden: true}, 
             ...
             //{name : 'disseteNum',index : 'disseteNum',align:'left',editable:true,editrules:{number:true},formatter:'integer',sortable: false,editoptions:{readonly:false,onkeyup:"checkInput.clearNoNum(this,10);"}},
...
function Summary(){
    ...
    $.each(ids,function(i,value){
    	...
    	$("#dataGrid").jqGrid('setCell', value ,"dissetePrice" ,currRow.dissetePrice);//currRow.disseteAmount / currRow.disseteNum
...
//		obj.remark = "";
//		obj.seteNum = parseInt(obj.instroNum - obj.noSeteNum)
