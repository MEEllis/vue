...
function print(){
	...
	function todo(){
		$.printBills(basePath + '/funds/print/entrustChangePrice', 
			{
		      billsId:id,
...
function queryPage(){
	...
	//后台查询
	$.request({
		url: this.basePath + '/IcashMain/page/wttj_query',
		type : "post",
		dataType : 'json',
		data:model,
...
function initPageData(bills){
	...
	var formObj = bills;
	var mingxiList = bills.cashEntrustModpriList;
...
function saveBtClick(){
	...
	//添加一些代码
	for (var int = 0; int < model.cashEntrustModpriList.length; int++) {
		var deta  = model.cashEntrustModpriList[int];
		if(deta.noSeteNum <= 0){
			$.MsgBox('保存提示','结算量不能为0');
			return;
		}
	...
	//code......
	billsDao.saveEntrustChangePriceBills(model, function(data){
		if(data.result == 1){
			$.MsgBox('提示','操作成功');
...
function getPageData(){
	...
	//明细
	formObj.cashEntrustModpriList = dataGrid.getGridDataList();
...
function selectBeentrustStockReferenceOpen(){
	...
	callBack = function(){
		...
		for ( var i = 0; i < models.length; i++) {
			...
			model.index = (ids.length+1);
			model.op = "OP";
			model.remark = "";
//			model.ajustPrice2 = model.ajustPrice;
...
function initDataGrid(){
	...
	var paras = {
	    ...
	    colNames:['id','出库日期','商品ID','商品名称','出库量', '未结量','串号','辅助串号','出库价','本次调价前价','本次调价差额','本次调价后价', '备注', '备注'], 
	    colModel:
