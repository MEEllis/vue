...
function print(){
	...
	function todo(){
		$.printBills(basePath + '/funds/print/entrustSettlement', 
			{
		      billsId:id,
...
function queryPage(){
	...
	//后台查询
	$.request({
		url: this.basePath + '/IcashMain/page/wtjs_query',
		type : "post",
		dataType : 'json',
		data:model,
...
function initPageData(bills){
	...
	var formObj = bills;
	var mingxiList = bills.cashEntrustSetedtlList;
...
function saveBtClick(){
	...
	//添加一些代码
	for (var int = 0; int < model.cashEntrustSetedtlList.length; int++) {
		var deta  = model.cashEntrustSetedtlList[int];
		if(deta.seteNum <= 0){
			$.MsgBox('保存提示','结算量不能为0');
			return;
		}
	...
	//添加一些代码
	//code......
	billsDao.saveEntrustSettlementBills(model, function(data){
		if(data.result == 1){
			$.MsgBox('提示','操作成功');
...
function getPageData(){
	...
	//明细
	formObj.cashEntrustSetedtlList = dataGrid.getGridDataList();
					...
					if(data.data.dataList.length != 0){
						for(var i=0;i<data.data.dataList.length;i++){
							ulHtml += '<li onclick = "imeiUlLiClick({'+
					          ...
					          'outstroNum:\''+data.data.dataList[i].outstroNum+'\','+
					          ...
					          'outstroDate:\''+data.data.dataList[i].outstroDate+'\','+
...
function wResize(){
	...
	$("#entrustStockReferenceFrame").height(winH*0.75);
...
//打开受托库存引用对话框
function selectEntrustStockReferenceOpen(){
	if($("#slideThree").is(':checked'))return;
	
	if($("input[name='sectionId']").val() == "" || $("input[name='contactsunitId']").val() == ""){$.MsgBox("操作提示","部门或往来单位未选择");return;}
	$('#entrustStockReferenceModal').modal('show').find('.referenceFrame').css({
	    height: $("html").height()/1.25
	});
	
	//此处应用的 是未结算的数据 所以在这里需要添加查询
	entrustStockReferenceFrame.reLoadGrid({status:2,sectionId:$("input[name='sectionId']").val(),contactsunitId:$("input[name='contactsunitId']").val(),
        contactsunitName:$("input[name='contactsunitName']").val()});
	
	callBack = function(){
		if(arguments[0].length == 0){
			$('#entrustStockReferenceModal').modal('hide');
			return ;
		}
		...
		for ( var i = 0; i < models.length; i++) {
			...
			model.setePrice = model.ajustPrice;
			if(model.ifManageImei == "true"){
				model.seteNum = 1;
			}else{
				model.seteNum = $.parseInt(model.noSeteNum);
			}
...
function Summary(){
    ...
    $.each(ids,function(i,value){
    	...
    	if(currRow.noSeteNum-currRow.seteNum<0){
			$.MsgBox("提示","结算数量不能大于未结算量!");
			$("#dataGrid").jqGrid('setCell', value ,"seteNum", currRow.noSeteNum);
		}
    	var currRow = $("#dataGrid").jqGrid('getRowData', value);
...
function initDataGrid(){
	...
	var paras = {
	    ...
	    colNames:['id', '出库日期','商品ID','商品名称','出库数量', '未结算量','串号','辅助串号','入库价','入库调价后价','本次结算数量','结算价','结算总额','结算后剩余数量', '备注', '备注'], 
	    colModel:
	    	[ {name : 'id',index : 'id',align:'left',sortable: false,hidden: true}, 
             {name : 'outstroDate',index : 'outstroDate',align:'left',sortable: false,formatter:formatterDate}, 
             ...
             {name : 'ajustPrice',index : 'ajustPrice',align:'left',editable:false,editrules:{number:true},formatter:'number',sortable: false},
             {name : 'seteNum',index : 'seteNum',hidden: false,editable:true,editrules:{number:true},formatter:'integer',sortable: false,editoptions:{readonly:false,onkeyup:"checkInput.clearNoNum(this,10);"}},
             {name : 'setePrice',index : 'setePrice',hidden: false,editable:false,sortable: false,formatter:'number',editoptions:{onkeyup:"checkInput.checkNum(this,12)"}},
             {name : 'seteAmount',index : 'seteAmount',hidden: false,editable:false,sortable: false,formatter:'number'},
             {name : 'numberRemaining',index : 'numberRemaining',hidden: false,editable:false,sortable: false,formatter:'integer'},
	...
	var callBackList = {		
		onCellSelect:function(rowid,iCol,cellcontent,e){
			...
			if(iCol == 12){
		      ...
		      if($.trim(currRow.imei) == ""){
		    	  $("#dataGrid").setColProp("seteNum",{editable:true});
		      }else{
		    	  $("#dataGrid").setColProp("seteNum",{editable:false});
		      }
        ...
        getGridDataList:function(rows){
        	...
        	return $.map(rows,function(row){
				if(row.goodsName != ""){
		    		row.outstroNum = parseInt(row.outstroNum) ;
