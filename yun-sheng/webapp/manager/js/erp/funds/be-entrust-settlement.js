...
function queryPage(){
	...
	//后台查询
	$.request({
		url: this.basePath + '/IcashMain/page/stjs_query',
		type : "post",
		dataType : 'json',
		data:model,
...
function initPageData(bills){
	...
	var formObj = bills;
	var mingxiList = bills.cashBeentrustSetedtlList;
...
function saveBtClick(){
	...
	//添加一些代码
	for (var int = 0; int < model.cashBeentrustSetedtlList.length; int++) {
		var deta  = model.cashBeentrustSetedtlList[int];
		if(deta.seteNum <= 0){
			$.MsgBox('保存提示','结算量不能为0');
			return;
		}
	...
	//添加一些代码
	//code......
	billsDao.saveBeEntrustSettlementBills(model, function(data){
		if(data.result == 1){
			$.MsgBox('提示','操作成功');
...
function getPageData(){
	...
	//明细
	formObj.cashBeentrustSetedtlList = dataGrid.getGridDataList();
...
function initDataGrid(){
	...
	var paras = {
	    ...
	    colNames:['ID','入库日期','商品ID','商品名称','入库数量', '未结算量','串号','辅助串号','入库价','入库调价后价','本次结算数量','结算价','结算总额','结算后剩余数量', '备注', '备注'], 
	    colModel:
