...
var lendSaleList = new Array(); //借入归还单集合
...
$(function() {
	...
	initLendSale(); //引入未归还表格
	lendSaleModal(); //借入转采购表格
	filterSelect(); //查询数据
...
function onClickDelete() {
	if (ibillsMainCC != null && ibillsMainCC != undefined && ibillsMainCC.id != undefined && ibillsMainCC.id != null && ibillsMainCC.billsCode == undefined) {
		$.zxsaas_plus.showconfirm("", "是否删除当前单据?", function() {
			storageObj.deleteIbillsMainDraftCC(ibillsMainCC.id, 8, function(data) {
...
function lendSaleGzHc(num) {
	...
	storageObj.lendSale(ibillsMainCC.id, 'JCZXS', num, $('#hcTime').val(), function(data) {
		var str = data.data.str;
...
function filterSelect() {
	...
	storageObj.filterLendSaleDetail(num, pageNumber, statusBills, function(data) {
...
function assign() {
	...
	$('#ibillsHeadBorrowBack').clearForm(true);
	jQuery("#jqGrid_borrow").jqGrid("clearGridData");
	lendSaleList = ibillsMainCC.lendSaleList;
	//赋值表头
	var params = $('#ibillsHeadBorrowBack').formToArray();
	$(params).each(function(index, element) {
	...
	//赋值表体
	if (lendSaleList != undefined && lendSaleList.length != 0) {
		for (var i = 0; i < lendSaleList.length; i++) {
			if (importNumId2 != '') importNumId2 += ',';
			importNumId2 += lendSaleList[i].id; //添加   单据数量表id
			lendSaleList[i].borrowLendDateString = lendSaleList[i].lendDateString;
			lendSaleList[i].borrowLendNum = lendSaleList[i].borrowNum;
			$("#jqGrid_borrow").jqGrid('addRowData', i, lendSaleList[i]);
		}
	}
	//赋值表尾
...
function draftBillsInsert2() {
	...
	ibillsMainCC.billsType = '62';
	//获取表体
	ibillsMainCC.lendSaleList = new Array();
	$(rowIds).each(function(index, element) {
		...
		ibillsMainCC.lendSaleList[element] = rowData;
	});

	//新增
	storageObj.saveNewLendSaleDetailDraft(ibillsMainCC, function(data) {
...
function draftBillsImport2(copyString) {
	...
	//修改草稿
	storageObj.saveLendSaleDetailDraft(ibillsMainCC, function(data) {
		var obj = data.data.row;
		if (obj != null) {
			if (copyString != 'copy') {
...
function billsUpdate2(copyString) {
	...
	$(rowIds).each(function(index, element) {
		...
		if (ibillsMainCC.lendSaleList[element] != undefined) {
			ibillsMainCC.lendSaleList[element].remark = rowData.remark;
		}
	});
	//修改正式单据
	storageObj.saveToOfficial(8, ibillsMainCC, function(data) {
		var obj = data.data.row;
		if (obj != null) {
			if (copyString != 'copy') {
				$.zxsaas_plus.showalert("", "修改成功!");
...
function onclickImport(id) {
	...
	if (id != undefined && id != null && id != '') {
		...
		rowData.revertNum = rowData.surNum; //本次转销售数
		rowData.revertImei = rowData.imei; //本次转销售串号
...
function lendSaleModal() {
	...
	var colNames = ['操作', '借出单号', '借出日期', '商品名称', '借出数量', '串号', '已转销售数', '已归还数', '未归还数', '本次转销售数', '本次转销售串号', '单价', '金额', '未归还剩余数量', '备注', 'id', '草稿单据id', '仓库id', '商品id', '借入日期', '借入数量', '新增人', '新增时间', '是否串号管理'];
	var JqGridColModel = [{
		name: 'deliveryDo',
		index: 'deliveryDo',
		width: 70,
...
function initLendSale() {
	...
	var lock = false;
	var myobj = [];
	var colNames = ['借出单号', '借出日期', '商品名称', '借入数量', '单价', '金额', '串号', '已转销售数', '已归还数', '未归还数', '仓库id', '商品id', '是否串号管理'];
	var JqGridColModel = [{
		name: 'refMainId',
		index: 'refMainId',
		width: 100,
