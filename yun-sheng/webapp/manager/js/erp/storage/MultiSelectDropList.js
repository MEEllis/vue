...
//			var clazz = 'hide_text' + options.msg;
			...
			//1.下拉选项包裹
			var $container = $('<div class="container_select" style=""></div>');
			$container.width($this.width());
			$container.css({'marginLeft':'80px;'})
			//2.创建 全选和确认按钮  top层 
			var $top = $('<div class="top"></div>');//外层div包裹
			var $all = $('<input type="checkbox" class="select_all"/><span>'+options.selectallTxt+'</span>');//全选
			...
			for(var i = count-1; i >= 0; i--){
			   ...
			   var $list = $('<div><input type="checkbox" value='+options.ids[i]+' class="checkOne ' + options.msg +'" /><span>'+options.data[i]+'</span><br></div>');
			...
			$dropList.change(function (){//勾选选项
			 ...
			 var opt_arr = [];
			 var hide_arr = [];
	   		 opt_arr.push(options.defaultSelected);//存放值
	   		 hide_arr.push(options.defaultSelectedIds);//存放id
			 var $dropList_selected = $content.children().children('input:checked');
			 $ipt.val(opt_arr.join(';'));
			 $hide_text.val(hide_arr.join(',')); 	
			 var o = $all[0];
