...
function initial(){
	loadmodal({TableName:"#jqGrid_stockKepper",pager:"#jqGridPagerstockKepper",LoadTableUrl: "/manager/beginning/entrustim/selectList/2"});
}

//初始化的时候判断是否已执行
function init(){
	$.request({
        url: '/manager/beginning/entrustim/init/2', 
        type: "POST",
	    traditional: true,
	   ...
	   if(status==1){
		   $.zxsaas_plus.showalert("提示","抱歉,已禁用!");
			 ...
			 $.zxsaas_plus.showconfirm("执行","是否要执行?",function(){
						$.request({
					   		   url: '/manager/beginning/entrustim/saveInfo/2', 
					   		   type: "POST",
					   		   traditional: true,
					   		   success: function (data) {
					   						 if(data.result==1){
...
$(document).on('click' ,'.op-execute',function(){
	  if(status==0){
		   ...
		   $.zxsaas_plus.showconfirm("清空","是否要反执行?",function(){
				$.request({
			   		   url: '/manager/beginning/entrustim/recallSaveInfo/2', 
			   		   type: "POST",
			   		   traditional: true,
			   		   success: function (data) {
			   					     if(data.result==1){
...
$(document).on('click','.clear',function (e) {
	 if(status==1){
		 ...
		 $.zxsaas_plus.showconfirm("清空","是否要清空所有数据?",function(){
					$.request({
				   		   url: '/manager/beginning/entrustim/deleteAll/2', 
				   		   type: "POST",
				   		   traditional: true,
				   		   success: function (data) {
				   						 if(data.result==1){
...
function loadmodal(options){
	/*初始化表格参数*/
     ...
     colNames = ['id','EXCEL顺序号','系统判定情况','往来单位编码','往来单位名称','商品编码','商品名称','部门编码','部门名称',
                 '串号','辅助串号','数量','出库总额','结算总额','出库日期','备注'];//委托入库
     JqGridColModel = [
                             ...
                             {name:'orderno',index:'orderno', width:70,align:'center',sortable:false},//Excel顺序号
							    ...
							    {name:'outstroDate',index:'outstroDate', width:100,align:'center',sortable:true},//入库日期
...
$(document).on("click",".lead_model",function(e){
	...
	if(fileExtension == "xls"){
		$.loading();
			$.ajaxFileUpload({
				url : "/manager/beginning/entrustim/import/2/",
				secureuri : false,
				fileElementId : "file",
				dataType : "json",
				success : function(data) {
