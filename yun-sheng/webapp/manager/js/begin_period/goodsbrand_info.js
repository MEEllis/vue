/*初始化页面*/
function initial(){
	loadmodal({TableName: "#jqGrid_branInfo",pager:"#jqGridPagerBranInfo",LoadTableUrl: "/manager/beginning/goodsbrand/selectList/1"});
}
...
function loadmodal(options){
	/*初始化表格参数*/
        var  colNames =[];
        var JqGridColModel =[];
        colNames= ['id',"Excel顺序号",'系统判定结果','品牌 编码','品牌名称','备注'];//品牌资料
        JqGridColModel = [
								...
								{name:'code',index:'code', width:100,align:'center',sortable:false},//编码editoptions:{dataEvents:[{type:"blur",fn:notValcume}]
								{name:'name',index:'name', width:100,align:'center',sortable:false},//名称
...
$(document).on('click','.clear',function (e) {
	...
	$.zxsaas_plus.showconfirm("清空","是否要清空所有数据?",function(){
		$.request({
            url: '/manager/beginning/goodsbrand/deleteAll/1', 
            type: "POST",
		    traditional: true,
...
$(document).on('click' ,'.execute',function(){
	$.zxsaas_plus.showconfirm("执行","是否确定执行?",function(){
		$.request({
            url: '/manager/beginning/goodsbrand/saveInfo/1', 
            type: "POST",
		    traditional: true,
            success: function (data) {
				 if(data.result==1){
...
$(document).on("click",".lead_model",function(e){
	...
	if(fileExtension == "xls"){
		$.loading();
		$.ajaxFileUpload({
			url : "/manager/beginning/goodsbrand/import/1",
			secureuri : false,
			fileElementId : "file",
			dataType : "json",
			success : function(data) {
