/*初始化页面*/
function initial(){
	loadmodal({TableName: "#jqGrid_branInfo",pager:"#jqGridPagerBranInfo",LoadTableUrl: "/manager/beginning/section/selectList"});
}
...
function loadmodal(options){
	...
	var  colNames =[];
    var JqGridColModel =[];
    colNames= ['id','Excel顺序号','系统判定结果','部门编码','部门名称','所属上级编码','所属上级名称','所属地区编码',
               '所属地区名称','部门属性编码','部门属性名称','是否为门店','启用日期','是否公开库存量','电话','地址','备注'];
    JqGridColModel = [
				...
				{name:'sectionCode',index:'sectionCode', width:100,align:'center',sortable:false},//仓库编码
				{name:'sectionName',index:'sectionName', width:100,align:'center',sortable:false},//仓库名称
				{name:'parentCode',index:'parentCode', width:100,align:'center',sortable:false},//仓库类型
				{name:'parentName',index:'parentName', width:100,align:'center',sortable:false},//部门编码
				{name:'regionCode',index:'regionCode', width:100,align:'center',sortable:false},//部门名称
				{name:'regionName',index:'regionName', width:100,align:'center',sortable:false},//业务员编码
				{name:'attrCode',index:'attrCode', width:100,align:'center',sortable:false},//业务员名称
				{name:'attrName',index:'attrName', width:100,align:'center',sortable:false},//是否为门店默认仓
			    {name:'ifStore',index:'ifStore', width:100,align:'center',sortable:false},//备注
				{name:'usedDate',index:'usedDate', width:100,align:'center',sortable:false},//备注
				{name:'isKcflag',index:'IS_KCFLAG', width:130,align:'center',sortable:false},//备注
...
$(document).on('click','.clear',function (e) {
	...
	$.zxsaas_plus.showconfirm("清空","是否要清空所有数据?",function(){
		$.request({
            url: '/manager/beginning/section/deleteAll', 
            type: "POST",
		    traditional: true,
...
//		ajax 执行数据 （完成弹出提示：更新成功XX条   更新失败XX条（页面显示失败的条数））
		$.request({
            url: '/manager/beginning/section/saveInfo', 
            type: "POST",
		    traditional: true,
            success: function (data) {
				 if(data.result==1){
...
$(document).on("click",".lead_model",function(e){
	...
	if(fileExtension == "xls"){
		$.loading();
		$.ajaxFileUpload({
			url : "/manager/beginning/section/import",
			secureuri : false,
			fileElementId : "file",
			dataType : "json",
			success : function(data) {
