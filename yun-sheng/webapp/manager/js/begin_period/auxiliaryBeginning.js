...
function loadmodal(options){
	...
	var  colNames =[];
    var JqGridColModel =[];
    colNames= ['id','Excel顺序号','系统判定结果','科目编码','科目名称','往来单位编码','往来单位名称','部门编码','部门名称',
               '职员编码','职员名称','借方累计','贷方累计','方向','期初余额','业务部门编码','业务部门名称'];
    JqGridColModel = [
				...
				{name:'subjectCode',index:'subjectCode', width:130,align:'center',sortable:false},//类型
				{name:'subjectName',index:'subjectName', width:130,align:'center',sortable:false},//编码
				{name:'contactUnitCode',index:'contactUnitCode', width:130,align:'center',sortable:false},//名称
				{name:'contactUnitName',index:'contactUnitName', width:130,align:'center',sortable:false},//开户行
				{name:'sectionCode',index:'sectionCode', width:130,align:'center',sortable:false},//银行户名
				{name:'sectionName',index:'sectionName', width:100,align:'center',sortable:false},//银行账号
				{name:'jobCode',index:'jobCode', width:130,align:'center',sortable:false},//刷卡最高额
				{name:'jobName',index:'jobName', width:100,align:'center',sortable:false},//刷卡最低额
				...
				{name:'direction',index:'direction', width:70,align:'center',sortable:false},//最低手续费
				{name:'periodBalance',index:'periodBalance', width:100,align:'center',sortable:false},//备注
				{name:'depYeCode',index:'depYeCode', width:130,align:'center',sortable:false},//最低手续费
				{name:'depYeName',index:'depYeName', width:130,align:'center',sortable:false}//最低手续费
			 ];
		var defaults = {
		LoadTableUrl: "",
		pager:"",
...
function initial(){
	loadmodal({TableName: "#jqGrid_account",pager:"#jqGridPagerAccount",TableInfo:"0",LoadTableUrl: "/manager/beginning/cw/selectList/2"});
}
...
$(document).on('click','.clear',function (e) {
	...
	$.zxsaas_plus.showconfirm("清空","是否要清空所有数据?",function(){
		$.request({
            url: '/manager/beginning/cw/deleteAll/2', 
            type: "POST",
		    traditional: true,
...
//		ajax 执行数据 （完成弹出提示：更新成功XX条   更新失败XX条（页面显示失败的条数））
		$.request({
            url: '/manager/beginning/cw/saveInfo/2', 
            type: "POST",
		    traditional: true,
            success: function (data) {
				 if(data.result==1){
...
$(document).on("click",".lead_model",function(e){
	...
	if(fileExtension == "xls"){
		...
		$.ajaxFileUpload({
			url : "/manager/beginning/cw/import/2",
			secureuri : false,
			fileElementId : "file",
			dataType : "json",
			success : function(data) {
