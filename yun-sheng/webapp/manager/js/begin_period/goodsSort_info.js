/*初始化页面*/
function initial(){
	loadmodal({TableName: "#jqGrid_branInfo",pager:"#jqGridPagerBranInfo",LoadTableUrl: "/manager/beginning/goodsbrand/selectList/3"});
}
...
function loadmodal(options){
	/*初始化表格参数*/
	 ...
	 var  colNames =[];
		var JqGridColModel =[];
		colNames= ['id','Excel顺序号','系统判定结果','上级类别编码','上级类别名称','本级类别编码','本级类别名称','备注'];
		JqGridColModel = [
					...
					{name:'parentCode',index:'paerntCode', width:100,align:'center',sortable:false},//上级类别编码
					{name:'parentName',index:'paerntName', width:100,align:'center',sortable:false},//上级类别名称
					{name:'code',index:'code', width:100,align:'center',sortable:false},//本级类别编码
					{name:'name',index:'name', width:100,align:'center',sortable:false},//本级类别名称
...
$(document).on('click','.clear',function (e) {
	...
	$.zxsaas_plus.showconfirm("清空","是否要清空所有数据?",function(){
		$.request({
            url: '/manager/beginning/goodsbrand/deleteAll/3', 
            type: "POST",
		    traditional: true,
...
//		ajax 执行数据 （完成弹出提示：更新成功XX条   更新失败XX条（页面显示失败的条数））
		$.request({
			url: '/manager/beginning/goodsbrand/saveInfo/3', 
            type: "POST",
		    traditional: true,
            success: function (data) {
				 if(data.result==1){
...
$(document).on("click",".lead_model",function(e){
	...
	if(fileExtension == "xls"){
		$.loading();
		$.ajaxFileUpload({
			url : "/manager/beginning/goodsbrand/import/3",
			secureuri : false,
			fileElementId : "file",
			dataType : "json",
			success : function(data) {
