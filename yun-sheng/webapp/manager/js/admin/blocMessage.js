...
//			    console.log(event + ',' + treeId + ',' + treeNode.tId + ", " + treeNode.name + "," + treeNode.checked);
			 ...
			 $.ajax({
		            ...
		            success: function (data) {
		                ...
		                var list = $('.center_docu').next();
		                console.log($('.center_docu').next());
						
						$('.center_docu').each(function(){
							console.log($(this));
							console.log('................;;;');
...
//						zTree.checkNode(node);
	...
	(function(){
		...
		function zTreeOnCheck(event, treeId, treeNode) {
			...
			$('.update_tree_ids').val(ids);//ids存放于此
		}; 
		 ...
		 $.ajax({
	            ...
	            success: function (data) {
	                ...
	                zTree.expandAll(true);//展开全部节点
					var node = zTree.getNodeByParam("id",111);//选中某节点
					//zTree.selectNode(node);
					zTree.checkNode(node);
...
//		inputbox: ".inputbox" //搜索条件框存放容器。遵照css选择器书写
			...
			var colNames = ['操作','ID','集团编码','集团名称','管理员工号','重置管理员密码','是否禁用','备注','新增人','新增时间','修改人','修改时间'];
			var JqGridColModel=[
