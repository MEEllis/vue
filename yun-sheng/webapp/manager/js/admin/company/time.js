...
//		inputbox: ".inputbox" //搜索条件框存放容器。遵照css选择器书写
			...
			var colNames = ['ID','公司名称','分期商名称','佣金比例','是否禁用','备注','新增人','新增时间','修改人','修改时间'];
			var JqGridColModel=[
								...
								{name:'storageName',index:'storageName', width:150,align:'center', sorttype:"string",formatter:"currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",defaulValue:"0",decimalPlaces:"3",suffix:"%"},editable:true,sortable:false},
...
//					(gridData.depDouble == '1') ? ($('.depDouble').prop({'checked':''})) : ($('.depDouble').prop({'checked':'checked'}));
					...
					$.zxsaas_plus.showalert("","佣金比例修改成功!");
					return true;
				}
			});
			...
			$('.accredit').click(function(){
				...
				//console.log(ids + '....ids');
				if(ids.length < 1){
					$.zxsaas_plus.showalert("错误","请勾选公司后再授权!");
					return false;
				}else if(ids.length >= 2){
					$.zxsaas_plus.showalert("错误","一次只能对一个公司进行授权!");
					return false;
				}else{
