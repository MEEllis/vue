		...
		LoadTableUrl: "../../../json/admin/company/Class.json",
...
//		inputbox: ".inputbox" //搜索条件框存放容器。遵照css选择器书写
		...
		$("#datetimepickerStart").datetimepicker({
	      ...
	      todayButton:false    //关闭选择今天按钮
			...
			var colNames = ['ID','公司名称','收支类型','类别名称','是否禁用','备注','新增人','新增时间','修改人','修改时间'];
			var JqGridColModel=[
								...
								{name:'deviceNo',index:'deviceNo', width:200,align:'center', sorttype:'string',formatter:'select', editoptions:{value:"0:收入;1:支出"},sortable:false},
...
//					(gridData.deviceNo == '0') ? ($("input:radio[value='0']").attr('checked','true')) : ($("input:radio[value='1']").attr('checked','true'));
					if(gridData.deviceNo == '0'){
...
//						$("input:radio[value='0']").attr('checked',false);
			...
			//  0        收入
			//  1        支出
			$('.radio_Class').on('click',function(e){
				var str = $("input[name='inlineRadioOptions']:checked").val();  //获取选中的值
				console.log(str);
				var selectedRowIds = $(options.TableName).jqGrid("getRowData");  //获取表格数据
				var len = selectedRowIds.length + 1;  
				if(str == '0'){
					for(var i = 1;i <= len ;i ++) {  
						var gridData = $(options.TableName).jqGrid("getRowData",i);
//						if(gridData.deviceNo == '1'){
//							$(options.TableName).setRowData(i,null,{display: 'none'});//隐藏支出
//						}
						(gridData.deviceNo == '1') ? ($(options.TableName).setRowData(i,null,{display: 'none'})) : ($(options.TableName).setRowData(i,null,{display: 'table-row'}));
					}  
				}else{
					for(var i = 1;i <= len ;i ++) {  
						var gridData = $(options.TableName).jqGrid("getRowData",i);
//						if(gridData.deviceNo == '0'){
//							$(options.TableName).setRowData(i,null,{display: 'none'});//隐藏收入
//						}
						(gridData.deviceNo == '0') ? ($(options.TableName).setRowData(i,null,{display: 'none'})) : ($(options.TableName).setRowData(i,null,{display: 'table-row'}));
					}  
				}
