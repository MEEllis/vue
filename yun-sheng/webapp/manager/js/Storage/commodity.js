...
function loadmodal() {
	...
	var colNames = ['操作', 'ID', '商品名称', '移出数量', '备注'];
	var JqGridColModel = [{
		name: 'deliveryDo',
		index: 'deliveryDo',
		width: 70,
	...
	function totalCalc() {
		...
		for (var i = 0, len = idsLeft.length; i < len; i++) {
			var thisid = $(options.TableName).getCell(idsLeft[i], "numberPu");
	...
	//保存
	$(document).on('click', '.redDashed', function(e) {
	...
	function numberModel(cellvalue, options, rowObject) {
		...
		if (rowObject.numberModel == '0') {
			str = '<input type="text" id="checkNum" class="numberRid' + options.rowId + '" style="border:0;text-align:center;width:70px" value="' + cellvalue + '" /><span class="numberPlus glyphicon glyphicon-plus" data-toggle="modal" data-target="#numberChoose" data-rId="' + options.rowId + '"></span>';
		} else {
			str = '<input type="text" id="checkNum" class="numberRid' + options.rowId + '" style="border:0;text-align:center;width:90px" value="' + cellvalue + '" />';
		}
		return str;
	};
	 ...
	 */
	//			function footerData(){			
	//				var numberPu = $(options.TableName).getCol('numberPu',false,'sum');
	//				
...
$(document).on('click', '.numberPlus', function(e) {
	...
	//	console.log(rowData);
	var endGoods = check(rowData.spmc);
	//var endStor = check(rowData.numberPu);
	$('.goodsMsg').val(endGoods);
	$('.storageMsg').val($('.shiftOut')).html();
