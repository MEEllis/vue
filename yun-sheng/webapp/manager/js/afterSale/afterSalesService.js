...
function loadmodal(options){
	...
	var JqGridColModel = [];
	var JqGridColModel0 =[
						{name:'deliveryDo',index:'deliveryDo', width:70,align:'center',sortable:false,formatter: addAndDelete,hidden:true},
	                   ...
	                   {name:'storageId',index:'storageId', width:150,align:'center',sorttype:'string',sortable:false},
	                   ...
	                   {name:'imei',index:'imei', width:150,align:'center',sorttype:'string',sortable:false,editoptions:{dataEvents:[{type:"blur",fn:saveCell}]}},
	                   {name:'falutDesc',index:'falutDesc', width:150,align:'center',sorttype:'string',sortable:false,editoptions:{dataEvents:[{type:"blur",fn:saveCell}]}},
	                   {name:'looksDesc',index:'looksDesc', width:130,align:'center',sorttype:'string',sortable:false,editoptions:{dataEvents:[{type:"blur",fn:saveCell}]}},
	                   {name:'suggestHandleMode',index:'suggestHandleMode', width:120,align:'center',sorttype:'string',sortable:false,formatter:'select',editoptions:{value:"1:自修;2:返厂;3:外修;4:换机;5:退货;"}},
	                   {name:'remark',index:'remark', width:150,align:'center',sorttype:'string',sortable:false,editoptions:{dataEvents:[{type:"blur",fn:saveCell}]}},
	...
	var select_name='';
	var defaults = {
	LoadBtnUrl: "/manager/afterSalesService/serviceHandle/selectAfterSaleHandleMain", //按钮工具栏加载地址
//	LoadTableUrl: "../../json/afterSale/registrationCard.json",
//	LoadTableUrl: "/manager/afterSalesService/serviceHandle/query",
...
//						}
		...
		function goodsModel(cellvalue, options, rowObject){
			return '<input type="text" class="godYF goodsRid' + options.rowId +'" style="border:0;text-align:center;width:150px;height:30px;" /><span class="spmcChoose glyphicon glyphicon-plus" data-toggle="modal" data-target="#spmcChoose" data-rId="' + options.rowId +'"></span>';
		   	...
		   	success:function(data){
					...
					for(var i = 0,len = dataList.length;i<len;i ++){
						...
						var arr={
								...
								'purchaseDateString':dataUp?dataUp.purchaseDateString : '',
...
$(document).on('click','.spmcCheck',function(cellvalue, options,rowObject){
	...
	$("#jqGrid_YFtab1").jqGrid("saveCell",lastrow ,lastcell );
	$('#jqGrid_YFtab1').jqGrid('setCell',rowsId,'name',a3);
...
$(document).on('click','.referSave',function(){
	...
	$.request({
		...
		type:'POST',
		success:function(data){
//			(data.result == 1) && ($.zxsaas_plus.showalert("","转至成功!"))
//			if(data.result == -999){
//				$.zxsaas_plus.showalert("错误","转至失败!")
//			}
			if(data.result == 1){
