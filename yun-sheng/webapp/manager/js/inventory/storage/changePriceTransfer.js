...
$(function(){
	...
	EventList.on({
		...
		formalSignNull:function(){//正式未查询到数据状态
			$('.slideThree').css('background','#8BAEC7');
			$('#billsHeader input,#billsHeader select,#collapseExample input,#otherOutstroNumIdStr,.imeiImport,#deliver,#del,#abandon,#save,#print,#copy').prop('disabled',true);
			$('.inputAssPlus,.del,#add').css({
				'width':0,
				'overflow':'hidden'
			})
    ...
    function initMenu(){
        ...
        $('#new').click(function(){
            ...
            if($('#slideThree').is(':checked')){
                window.location.href = basePath+'/inventory/storage/changePrice/transfer/main';
            }else{
            ...
            function changePrice(){
                if(canSave){
                    if($('#isDraft').val()==1){//草稿
                        ...
                        $.request({
                            type:'post',
                            url:basePath+'/inventory/storage/changePrice/transfer/saveDraftOrder/'+urlParam,
                        ...
                        $.request({
                            type:'post',
                            url:basePath+'/inventory/storage/changePrice/transfer/updateOfficialOrder',
                            data:{order:JSON.stringify(obj)},
                            dataType:'json',
                            success:function(data){
                                if(data.result==1){
        ...
        $("#del").click(function(){
            $.zxsaas_plus.showconfirm('提示','确定删除此草稿单?',function(){
                $.request({
                    type:'post',
                    url:basePath+'/inventory/storage/changePrice/transfer/deleteDraftOrder',
        ...
        $('#deliver').click(function(){
            ...
            if(flag==true){
                $.zxsaas_plus.showconfirm('提示','单据中存在价格为0的商品，是否确认发货',function () {
                    changePriceFa()
                },function () {
                    return;
                })
            ...
            function changePriceFa(){
                $.request({
                    type:'post',
                    url:basePath+'/inventory/storage/changePrice/transfer/executeDraftOrder',
                    data:{
                        id:$('#billsId').val(),
                        isDraft:$('#isDraft').val()
                    },
        ...
        $('.sureRed').click(function(){
            $.request({
                type:'post',
                url:basePath+'/inventory/storage/changePrice/transfer/redOfficialOrder',
                data:{
                    id:$('#billsId').val(),
        ...
        $('.printOpt').click(function(){
            var templateURL= basePath + '/inventory/storage/changePrice/transfer/print';
            var paras= {
                isDraft:$('#isDraft').val(),
                billsId:$('#billsId').val(),
    ...
    function initTopForm(){
        ...
        $("#inDepartmentName").comModalsSection({
            clickback:function(arrList){
                if(arrList.length===1){
                    var sectionId=arrList[0].dataId
                    var goodsId=[]
                    $("#grid").jqGrid("saveCell", lastrow, lastcell);
                    var ids=$('#grid').getDataIDs();
                    $.each(ids,function(index,rowid){
                        var idxRowData=$('#grid').jqGrid("getRowData",rowid);
                        if($.trim(idxRowData.goodsId)!=""){
                            goodsId.push(idxRowData.goodsId)
                        }
                    })
                    if(goodsId.length>=1){
                        goodsId=functionObjExtent.unique(goodsId)
                        $.ajaxPackage({
                            url:"/manager/inventory/common/getGoodsSectionCheckPriceList",
                            data:{
                                sectionId:sectionId,
                                ...
                                $.each(ids,function(index,rowid){
                                    ...
                                    if($.trim(idxRowData.goodsId)!=""){
                                        for(var i=0;i<dataList.length;i++){
                                            ...
                                            if(dataItem.goodsId==idxRowData.goodsId && Number(dataItem.sectionCheckPrice)!=0){
                                                $('#grid').jqGrid("setCell",rowid,"price",dataItem.sectionCheckPrice);
                                                break
                                            }
                                        }
                                        gridSum()
            ...
            btnGroupLeft: {
                ...
                printDropdown:{
                    ...
                    list:[{
                        name:'商品汇总（显示价格）',
                        click:function(){
                            print('numberDetail','true')
                        }
                    },{
                        name:'商品明细（显示价格）',
                        click:function(){
                            print('imeiDetail','true')
                        }
                    },{
                        name:'商品汇总（不显示价格）',
                        click:function(){
                            print('numberDetail','false')
                        }
                    },{
                        name:'商品明细（不显示价格）',
                        click:function(){
                            print('imeiDetail','false')
                        }
                    }]
                },
                copy:{
    ...
    function initImeiDr(){
                ...
                if($('#outDepartmentName').data('id')==null){
                    ...
                    $.request({
                        ...
                        data:{
                            ...
                            moveInSectionId:$('#inDepartmentName').data('id'),
                            queryKey:$(this).val()
                        },
                        success:function(data){
                            if(data.result==1){
        ...
        function imeiDr(opt){
            var def = {
                gridConfig: {
                    colNames: ['id','商品id','仓库id','仓库名称','商品类别', '操作', '串号', '辅助串号', '商品编码', '商品名称','商品品牌','型号'
                        ,'颜色', '备注','库存量','是否串号管理','税率','串号列表','引入订单表id','sectionCheckPrice','transferPrice'],
                    colModel: [
                        ...
                        {name : 'sectionCheckPrice',hidden:true},
                        {name : 'transferPrice',hidden:true},
                    ],
                }
            };
            opt = $.extend({}, def, opt);
...
//								判断是否已存在
    ...
    function initTable(){
        $('#grid').jqGrid({
            datatype: "local",
            mtype: 'POST',
            colNames:['操作', '<i class="bitianX">*</i>仓库名称','<i class="bitianX">*</i>商品名称','现库存', '<i class="bitianX">*</i>数量','<i class="bitianX">*</i>调拨单价','调拨总价','商品串号明细',
                '备注', '类别','商品编码', '品牌','型号','颜色',  '仓库id', 'goodsId','是否管理串号','明细id'],
            colModel:[
                ...
                {name: 'price', index: 'price',width: 100, align: 'right', sorttype: 'integer',formatter:'number',editable: true, sortable: false,
                    editoptions: {
                ...
                {name: 'amount', index: 'amount',width: 100, align: 'right', sorttype: 'integer',formatter:'number'},
            ...
            afterSaveCell: function(rowid,name,val,iRow,iCol){
                ...
                if(name == 'price' && currRow.goodsName != ""){
                    if($.parseInt(val) < 0){
                        //输入数量小于现库存
                        $.MsgBox('错误提示','不允许小于0！');
                        $('#grid').jqGrid('setCell', rowid ,"price" ,'0');
                    }
                }
                                        ...
                                        onDblClick:function(event, treeId, treeNode){
                                            if($(event.target).is('span')){
                                                if(treeNode.check_Child_State==-1){
                                                    $("#grid").jqGrid("saveCell",lastrow,lastcell);
                                                    var rowId = $("#grid").jqGrid("getGridParam", "selrow");
//                                    			$("#grid").jqGrid("setCell",rowId,"storageName",treeNode.name);
//                                    			$("#grid").jqGrid("setCell",rowId,"goodsName",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"stockCount",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"goodsNumber",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"imeiIds",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"remark",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"storageId",treeNode.storageId);
//                                    			$("#grid").jqGrid("setCell",rowId,"goodsId",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"ifManageImei",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"categoryName",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"code",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"brandName",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"models",'&nbsp');
//                                    			$("#grid").jqGrid("setCell",rowId,"color",'&nbsp');
        ...
        $(document).on('click','.spmc',function(){
            var rowId = $("#grid").jqGrid("getGridParam","selrow");
            var rowData = $("#grid").jqGrid("getRowData",rowId);
            var inDepartmentName= $.trim($('#inDepartmentName').data('id'))
            if(inDepartmentName==""){
                $.zxsaas_plus.showalert('error',"调入部门不能为空!");
                return
            }
            if(rowData.storageId!=''){
                ...
                $("#proTable").jqGrid('setGridParam', {
                    ...
                    postData:{
                        sectionId:$('#outDepartmentName').data('id'),
                        menuCode:$('#AUTH').data('code'),
                        moveInSectionId:inDepartmentName,
                        storageIds:rowData.storageId
                    }
                }).trigger("reloadGrid");
            }else{
    ...
    function initRender(){
        $('#proTable').jqGrid({
            ...
            colNames:['编码','商品名称', '类别', '品牌', '型号','颜色','是否串号管理', '计价方式','库存数','备注', '商品id','串号id','sectionCheckPrice','transferPrice'],
            colModel:[
	...
	function footDataRender(asyncOpt,param){//页面底部信息请求加载
		...
		$.request({
			async:asyncOpt.bool,
    		type:'post',
        	url:basePath+'/inventory/storage/changePrice/transfer/searchOrder',
        	data:param,
        	dataType:'json',
        	success:function(data){
				if(data.result==1){
	...
	function billSearch(url_Code,failCallback,billsId,callbackObj){//单据查询
		...
		$.request({
			async:false,
    		type:'post',
        	url:basePath+'/inventory/storage/changePrice/transfer/searchOrder?queryCodeStr='+url_Code,
        	data:filParam,
        	dataType:'json',
        	success:function(data){
				if(data.result==1){
    ...
    function addGoodName(proSelIds) {
        ...
        for(var i=0;i<proDataList.length;i++){
            ...
            var dataRow = {
                ...
                'price':proDataItem.transferPrice,
            };
            var flag=1;
            var cIndex;
