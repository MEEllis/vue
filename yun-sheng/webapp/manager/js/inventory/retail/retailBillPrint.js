    ...
    $.request({
        ...
        success:function(data){
            if(data.result==1){
                ...
                // 打印
                var printComObj = $('.printCom').comXiaoPiao({
                    billsId:id,
                    isJuHe:data.payResultVo==null?false:true
                })
                ...
                if(CODELIST.indexOf('B_LSKD_0020') == -1){
                    $('#AUTH').find('.printCom').hide();
                }else{
                    if($('#billsCode').val() ==null || $('#billsCode').val() =="" || $('#billsCode').val() ==undefined ){
                        $('.printCom').trigger('click.show');
                        $('#AUTH').find('.printCom').show();
                    }else{
                        $('#AUTH').find('.printCom').show();
                    }
                }
                ...
                if(operator.length<1){
                    ...
                    $.each(operator,function(i,item){
                        ...
                        item.debuBond = (item.debuBond == null)?"":item.debuBond;
                        ...
                        tr2+= '<tr><td>'+ (i+1) + '</td><td>'+ item.unitName +
                            ...
                            '</td><td>' + Number(item.qty) +'</td><td>'+  Number(item.commissionWill) +
