...
$(function () {
    var billsType=48   //单据类型 79:资金调整单 ,49:预收退款单,48:预付退款单,18:付款单 ,19:收款单,20:预付款单,21:预收款单,32:供应商返利单,33:客户保价单,34:客户返利单
    var tableUrl='/manager/inventory/fund/getFundPageList'
    init()
    function init() {
        getCustomProject('预付退款单-单据列表')
        initTopForm()
        ...
        $.pageListCommon.initMenuBtn({
            ...
            draftPost:function(mainIds){
                $.ajaxPackage({
                    url:'/manager/inventory/fund/inPayment/refund/excutePost/yftk_post',
                    data:{billsId:mainIds,billsType:81},
                    success:function (data) {
            ...
            red:function(postData){
                ...
                $.ajaxPackage({
                    url:"/manager/inventory/fund/inPayment/refund/excuteRed/yftk_red",
                    data:param,
                    success:function(data){
