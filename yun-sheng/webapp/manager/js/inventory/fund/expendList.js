...
$(function () {
            ...
            btnGroupLeft: {
                ...
                export: {
                    ...
                    click: function () {
                        ...
                        functionObjExtent.construtForm('/manager/inventory/expend/export', options)
                    }
                },
				add: {
					isShow: true,
					click: function () {
						window.parent.openWorkBoxByMenutext("费用单", '/manager/inventory/expend/expendBill', true);
					}
				},
                draftDel: {
                    isShow: true,
                ...
                copy: {
                    ...
                    click: function () {
                        ...
                        window.parent.openWorkBoxByMenutext('费用单',
                            '/manager/inventory/expend/expendBill?billsId='+billsId+'&copyFlag=1'+'&billsCode='+billsCode
                            ,true);
                    }
                },
                audit: {
    ...
    function selTable() {
        $('#rpGrid').jqGrid('setGridParam', {
            datatype:'json',
            url:"/manager/inventory/expend/selectList",
            postData:getSearchParam()
        }).trigger("reloadGrid");
    }

	getCustomProject('费用单据列表')
					...
					onCellSelect: function (rowid, index, contents, event) {
						if (def.gotoable) {
							...
							if (tarName== 'rpGrid_billsCode') {
								window.top.openWorkBoxByMenutext('费用单',   '/manager/inventory/expend/expendBill?billsId='+rowid+'&billsCode='+billsCode, true)
