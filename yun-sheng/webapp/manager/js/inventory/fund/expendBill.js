...
$(function () {
	...
	//切换摊销方式
	$('input[name="asd"]').change(function (e) {
		$('#allotGrid').resize()
		if (e.target.value == 0) {
			$('.monthBox').hide()
			$('#waitMonth').val('1')
	...
	function changeMonth() {
		...
		$.each(ids, function (k, v) {
			var amount = $('#dataGrid').getCell(v, 'amount')
			if ($('input[name="asd"]:checked ').val() == '0') {
				$('#dataGrid').setCell(v, 'avgAmount', amount);
			} else {
				$('#dataGrid').setCell(v, 'avgAmount', Math.floor(accDiv(amount * 1, $('#waitMonth').val().trim()) * 100) / 100);
			}
		})
	...
	function initMenuBtn() {
		var option = {
			btnGroupLeft: {
				...
				draftSave: {
					...
					click: function () {
                        ...
                        if(Number(data.settlementMode)==0){
                            if(payAmountSum!=0){
                                $.zxsaas_plus.showalert("提示",  "选择欠款,付款金额必须等于0!");
                                return false;
                            }
                        }else{
                            if(payAmountSum<=0){
                                $.zxsaas_plus.showalert("提示",  "选择现金结算,付款金额大于0!");
                                return false;
                            }
                        }
						$.ajaxPackage({
							contentType: 'application/json',
							url: '/manager/inventory/expend/saveBill',
				...
				draftPost: {
					...
					click: function () {
						...
						if (billsStatus != '1' || mainIds == "") {
							...
							$.ajaxPackage({
								type: 'post',
								url: '/manager/inventory/expend/postBill',
								data: {'id': mainIds},
								success: function (data) {
					...
					click: function () {
						...
						if (billsStatus != '1' || mainIds == "") {
							...
							$.zxsaas_plus.showconfirm("提示", "是否删除此草稿单？", function () {
								$.ajaxPackage({
									type: 'post',
									url: '/manager/inventory/expend/deteleBill',
									data: {'ids': mainIds},
									success: function (data) {
				...
				audit: {
					...
					click: function () {
						...
						if (billsStatus != '1' && mainIds != "" && isAudit == 0) {
							$.ajax({
								url: '/manager/inventory/expend/auditBill',
								type: 'post',
								traditional: true,
								dataType: 'json',
								data: {ids: mainIds},
					...
					click: function () {
						...
						if (billsStatus != '1' && mainIds != "" && isAudit == 1) {
							$.ajax({
								...
								traditional: true,
								dataType: 'json',
								url: '/manager/inventory/expend/unAuditBill',
								data: {'ids': mainIds},
								success: function (data) {
				...
				update: {
					...
					click: function () {
						...
						$.ajaxPackage({
							contentType: 'application/json',
							url: '/manager/inventory/expend/updateRemark',
							data: JSON.stringify(data),
							success: function (data) {
			...
			btnGroupRight: {
				history: {
					isShow: true,
					click: function () {
						window.parent.openWorkBoxByMenutext("费用单据列表", '/manager/inventory/expend/expendList', true);
					}
				}
			}
		};
		...
		$(".redSave").on("click", function () {
			var params = $("#billsHeaderForm").toJsonObject();
			$.request({
				url: "/manager/inventory/expend/redBill",
				dataType: "json",
				type: 'post',
				traditional: true,
				data: {
	...
	function initTable() {
		...
		var paras = {
			gridId: 'dataGrid',
			colNames: ['<i class="bitianX">*</i>类别名称', '支出类别id', '<i class="bitianX">*</i>费用归集部门', '费用归集部门id', '<i class="bitianX">*</i>经手人', '经手人id', '<i class="bitianX">*</i>金额', '每月摊销金额', '备注','id'],
			colModel: [
				{
				...
				{name: 'managersUid', width: 150, align: 'left', hidden: true},
				{
					name: 'amount',
					width: 150,
					align: 'right',
				...
				{name: 'avgAmount', width: 150, align: 'right'},
				{
		...
		var callBackList = {
			...
			afterEditCell: function (rowid, name, val, iRow, iCol) {//开始编辑
						...
						callback: function (data) {
							...
							$.each(data, function (k, v) {
								...
								if (lastrow + k < ids.length - 1) {
									...
									$('#dataGrid tr').eq(i).nextAll().each(function () {
										j++;
										$(this).attr("id", j);
				...
				if (iCol == '6') {
					...
					if ($('#dataGrid').getCell(rowid, 'sectionId')) {
						...
						$.zxsaas_plus.showalert('warning', '请先选择费用归集部门')
						$('#dataGrid').saveCell(lastrow, lastcell)
					}
			...
			afterSaveCell: function (rowid, name, val, iRow, iCol) {//保存编辑
				...
				if (iCol == '8') {
					$('#dataGrid').setCell(rowid, 'avgAmount', Math.floor(accDiv(val * 1, $('#waitMonth').val().trim()) * 100) / 100);
				}
				var avgAmount = $('#dataGrid').getCol('avgAmount', false, 'sum');

				$('#dataGrid').footerData("set", {
					"amount": '<font color="red" >' + Number(accountAmount).toFixed(2) + '</font>',
					// "avgAmount": '<font color="red" >' + Number(avgAmount).toFixed(2) + '</font>',
				}, false);
			},
	...
	function initAllotTable() {
		...
		var paras = {
			...
			colModel: [
				...
				{name: 'sectionId', width: 150, align: 'center', hidden: true},
				{
					name: 'managersName', width: 150, align: 'center', editable: true,
					edittype: 'custom',
					editoptions: {
						custom_element: my_input,
						custom_value: sectionName
	...
	$('.allotBtn').click(function () {
		...
		if (waitAllotMoney != '' && waitAllotMoney > 0) {
			if ($('#quickAllotModal input:radio:checked').val() == '0') {
				///平均分配
		...
		$.each(data, function (k, v) {
			if (v.allotMoney > 0) {
				dataGrid.kongRow = {
					...
					amount: v.allotMoney,
					avgAmount: Math.floor(accDiv(v.allotMoney, $('#waitMonth').val().trim()) * 100) / 100,
					remark: v.remark,
				}
				dataGrid.addKongRow()
			}
	...
	$('#avgMonth').click(function () {
		$('.avgBillCode').html($('#billsCode').val())
		$('#avgModal').modal('show')
	})
	...
	function initAvgTable() {
		$('#avgGrid').jqGrid({
			...
			viewrecords: true,
			sortorder: "desc",
			colNames: ['摊销月份', '摊销状态', '摊销金额', '操作', 'billsMainId'],
			footerrow: true,
			userDataOnFooter: true,
			colModel: [
				{name: 'avgMonth', index: 'avgMonth', align: 'left', sortable: false},
				{
					name: 'isAvg',
					index: 'isAvg',
					align: 'center',
					sortable: false,
					formatter: 'select',
					editoptions: {value: '0:未摊销;1:已摊销;'}
				},
				{name: 'amount', index: 'amount', align: 'right', sortable: false},
				{
					name: 'isSureAvg', index: 'isSureAvg', align: 'center', sortable: false, formatter:
					function (cellvalue, options, rowObject) {
						if (rowObject.isSureAvg == 1) {
							return '<a style="cursor: pointer" onclick="avgAction(' + options.rowId + ')">摊销</a>'
						} else {
							return ' '
						}
			...
			loadComplete: function (data) {
				$('#avgGrid').resize()
				var amount = $('#avgGrid').getCol('amount', false, 'sum');
				$('#avgGrid').footerData("set", {
					"amount": '<font color="red" >' + Number(amount).toFixed(2) + '</font>',
				}, false);
	...
	function gridSum() {
		...
		// var avgAmount = $grid.getCol('avgAmount', false, 'sum').toFixed(2);
		$grid.footerData("set", {
			amount: amount,
			// avgAmount:avgAmount
		});
	...
	function getParams() {
		...
		params.avgMonth = $('#waitMonth').val().trim()
		//明细数据
		var List = dataGrid.getGridDataList();
		params.cashOthExpendDetail = List
		var ids = $('#paymentGrid').getDataIDs()
	...
	function pageAjax(param, callbackObj) {
		$.ajaxPackage({
			type: 'post',
			dataType: 'json',
			url: '/manager/inventory/expend/loadBill',
			data: param,
			success: function (data) {
				if (data.result == 1) {
					...
					if (mainVo != null) {
						footerAdd = false
						fillTopData(mainVo)
						fillTable(mainVo.cashOthExpendDetail || [])
	...
	function fillTopData(billsHeader) {
		if (billsHeader.avgMonth > 1) {
			$('input[name="asd"][value="1"]').attr('checked', true)
			$('.monthBox').show()
		} else {
			$('input[name="asd"][value="0"]').attr('checked', true)
		}
	...
	function checkBillsStatus(mainVo) {
		...
		if (billsStatus == 1) {
			...
			$('#avgType').hide()
			$('.monthBox').hide()
			if ($('#avgMonthDetail').val() == '') {
				$('.avgMonthBox').hide()
			} else {
				$('.avgMonthBox').show()
				$('#avgMonth').text($('#avgMonthDetail').val())
			}
			if (billsStatus == 6) {
				$('.rightMap img:eq(0)').attr('src', '/manager/images/guozhang.png');
				$('#avgGrid').setGridParam({
					url: '/manager/inventory/expend/loadAvgByMainId',
					postData: {
				...
				$('#avgGrid').hideCol('isSureAvg')
				if(mainVo.billsCode.indexOf('_') != -1){
					$('#avgMonth').unbind('click')
				}
			}
			if (isAudit == 1) {
...
function avgAction(avgMonth) {
	...
	$.ajax({
		...
		data: {
			ids: ids,
			avgMonth: $('#avgGrid').getCell(avgMonth, 'avgMonth')
		},
		success: function (data) {
			if (data.result == '1') {
				$.zxsaas_plus.showalert('success', data.desc)
				setTimeout(function () {
					$('#avgGrid').jqGrid('setGridParam', {}).trigger("reloadGrid");
					$('#avgMonth').text($('#avgMonth').text().split('/')[0]*1+1+'/'+$('#avgMonth').text().split('/')[1])
				}, 1500)
			}else{
