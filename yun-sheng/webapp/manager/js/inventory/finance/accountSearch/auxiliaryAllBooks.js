$(function () {
    ...
    function initBindEvent() {
        ...
        $('#search').click(function () {
            ...
            var url = '/manager/finance/loadAuxiliaryLedgerInfo';
            ...
            if(searchParam.isEnableSub==1 || searchParam.isEnableUnit==1
                ...
                //载入 科目下拉框
                $.ajax({
                    url: "/manager/finance/loadAuxiliaryDetailTableFilter/3",
                    type: 'POST',
                    dataType: "json",
                    data:searchParam,
        ...
        commonModular.searchDetail(params,url, {
            ...
            callback:function (data) {
                ...
                $.each(info, function (i, detailItem) {
                    // 期初
                    if (detailItem.ifSumRow == 0) {
                        $("#rpGrid").jqGrid('setRowData', i + 1, {
                           // abstraction: "期初余额",
                        }, {"font-weight": 600});
                    }
                    //本月合计
                    else if (detailItem.ifSumRow == 1) {
                        $("#rpGrid").jqGrid('setRowData', i + 1, {
                            //abstraction: "本月合计"
    ...
    $("#export").click(function(){
        ...
        var isEnableSection = $("#searchQuery input[name='sectionInfoFilter']").is(":checked") ? "1" : "0";
        var isEnableEmployee = $("#searchQuery input[name='employeeInfoFilter']").is(":checked") ? "1" : "0";
        window.location.href= "/manager/finance/auxiliary/export/3?page=1&beginTime="+beginTime+'&endTime='+endTime+'&sectionIds='
