	  ...
	  $.fn.autCompleteTable=function(option){//自动完成筛选插件
		...
		return this.each(function(){
			...
			//	var multi = self.data("multi"); //是否单选，0为单选、1为多选
				...
				var val = $(valself).val(); //当前值
				var selfattr={};
				selfattr["l"] = self.offset().left; //获取控件右边定位
				selfattr["t"] = self.offset().top; //获取控件上边定位
				selfattr["h"] = self.outerHeight(); //获取控件高度
				selfattr["w"] = self.outerWidth(); //获取控件宽度
				var parentboxattr={};
				parentboxattr["l"] = option.parentbox.offset().left; //获取父窗口控件右边定位
				parentboxattr["t"] = option.parentbox.offset().top; //获取父窗口控件右边定位
...
//			self.keyup(function(event) {
//				if(self.val()==''&&event.keyCode!=13){
//					$('.input2seletlistbox').remove();
//				}else if(self.val()!=''&&event.keyCode!=13){
//					//keyup事件做模糊搜索
//					showtable(self.val());
//					event.preventDefault();
//					event.stopPropagation();
...
//			});
				function showtable(keyword) {//显示选择列表方法
					$(".input2tablebox").remove();
					$("body").append("<div class=\"input2tablebox\"  ><table class=\"input2seletlistbox zxsaastable\"  >\n"+option.thead+"\n<tbody></tbody>\n</table></div>"); //添加列表容器
					$.getJSON(src, {
						keyword: keyword
					}, function(r) {
						$.each(r.data.rows, function(i, item) {
						...
						for(var key in item)
							...
							$(".input2seletlistbox").append(li);
							$(".input2seletlistbox tr:last-child").append("<input type='hidden' class='text' value='"+item[namecol]+"' /><input type='hidden' class='value' value='"+item[valcol]+"' />");
						});
							var showboxattr={};
							$(".input2tablebox").show();
							showboxattr["h"] = $(".input2tablebox .input2seletlistbox").outerHeight(); //获取弹出窗口控件高度
							showboxattr["w"] = $(".input2tablebox").outerWidth(); //获取弹出窗口控件宽度
							showboxattr["l"] = selfattr["l"];
							showboxattr["t"]=0;
							if(selfattr.t+selfattr.h+showboxattr.h<parentboxattr.t+parentboxattr.h)
							{
