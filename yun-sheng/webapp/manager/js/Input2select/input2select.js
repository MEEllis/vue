...
;
				function showtable(keyword) {//显示选择列表方法
					$(".input2seletlistbox").remove();
					$(self).parent().append("<table class=\"input2seletlistbox\"  >\n"+option.thead+"\n<tbody></tbody>\n</table>"); //添加列表容器
					$.getJSON(src, {
						keyword: keyword
					}, function(r) {
						$.each(r.data.rows, function(i, item) {
						...
						for(var key in item)
							...
							$(self).parent().find(".input2seletlistbox").append(li);
							$(self).parent().find(".input2seletlistbox tr:last-child").append("<input type='hidden' class='text' value='"+item[namecol]+"' /><input type='hidden' class='value' value='"+item[valcol]+"' />");
						});
						$(self).parent().find(".input2seletlistbox").show();
						$(self).parent().find(".input2seletlistbox tr").unbind("click");
						$(self).parent().find(".input2seletlistbox tr").click(function(event) {
