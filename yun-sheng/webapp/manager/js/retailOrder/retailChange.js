...
//		treeGoodsModal('goodsDataTree','jqGrid_goodsMsgAdd','gridpager_goods','/manager/salesCommon/getGoods',false,rowId,'/manager/salesCommon/getGoods','#jqGrid_SubjectBalance');
	 ...
	 * 出库商品名称选择
	 */
	$(document).on('click', '.goodsOutName',function(e){
		//加载
		 var rowId = $(this).data('rid');
		treeGoodsModal('goodsDataTree','jqGrid_goodsMsgAdd','gridpager_goods','../../json/retailOrder/retailGoods.json',true,rowId,'../../json/retailOrder/goodsClazz.json','#jqGrid_swapOut');
	});
...
//				   return value.replace(/[^\d]$/g,'')
	...
	function loadmodalOut(urlData)
		{
		var options = {
			LoadBtnUrl: "../../json/button.json", //按钮工具栏加载地址
			LoadTableUrl: urlData,
			TableName: "#jqGrid_swapOut", //显示表格名称。遵照css选择器书写
			};
			...
			$.jgrid.defaults.styleUI = 'Bootstrap';	
			var lastsel='';//最后一次选中的行
			var colNames = ['操作','ID','赠品','出库仓库','商品名称','型号','颜色','串号','数量','标价','折扣率','折后价','折后金额','备注','gId','sttrs','是否串号管理'];
			var JqGridColModel=[
						{name:'deliveryDo',index:'deliveryDo', width:70,align:'center',formatter: addAndDelete,sortable:false},
						{name:'id',index:'id', width:200,align:'center', sorttype:'int',hidden:true,sortable:false},
						{name:'ifGift',index:'ifGift', width:80,align:'center', sorttype:'string',formatter:checkBox,sortable:false},
						...
						{name:'goodsNum',index:'goodsNum', width:100,align:'center',editable:true,sortable:false,formatter:"integer",editable:true,editoptions:{
							dataEvents:[{
								...
								fn:function(e){
									var rex=/^[0-9]*$/;
									if(rex.test($(this).val())){
										var zheHou = $(this).parent().next().next().next().html().replace(/\,/g,'');//折后价
										$(this).parent().next().next().next().next().html(($(this).val().replace(/\,/g,'') * zheHou).toFixed(2));
										totalAll.totalOut();
									}else{
										$.zxsaas_plus.showalert("提示","请输入合法数字！");
										$(this).val('');
						...
						{name:'money',index:'money', width:100,align:'center',  sorttype:'float',formatter:"currency",formatoptions: {decimalSeparator:".",defaulValue:"0",decimalPlaces:"3",suffix:"%"},editable:true,sortable:false,editoptions:{
							dataEvents:[{
								type:'blur',
								fn:function(e){
									var reg=/^[0-9]*\.{0,1}[0-9]{0,2}$/;
							...
							dataEvents:[{
								...
								fn:function(e){
									...
									if(reg.test($(this).val()) && ($(this).val() * 1) <= money){
										...
										$(this).parent().prev().html(($(this).val().replace(/\,/g,'') / biaoJia * 100).toFixed(2) + '%');//折扣率计算
...
//				chengben(maxid+1,'');
	...
	//删除一行
	$(document).on('click', '.trashOut',function(e){
		var thisTitle = $(this).attr("title");
		var rowId = $(this).parent().data('id');
		if(thisTitle == "删除行"){
			console.log('条数:' + $(options.TableName+' tbody tr').length);
...
//						}
	...
	//商品名称修改
	function goodsModel(cellvalue, options, rowObject){
		return '<input type="text" class="goodsRid' + options.rowId +'" style="border:0;text-align:center;width:170px" value="' + cellvalue +'" /><span class="goodsOutName glyphicon glyphicon-plus" data-toggle="modal" data-target="#goodsChoose" data-rId="' + options.rowId +'"></span>';
	...
	function loadmodalServe(urlData,tName,flag)
		...
		TableName: tName, //显示表格名称。遵照css选择器书写
			...
			var JqGridColModelDetail=[
								...
								{name:'times',index:'times', sorttype:'integer', width:120,align:'center',editable:true,sortable:false},
