...
<%@ include file="../Include/Header.jsp"%>
		...
		<script type="text/javascript">
			$(function() {
				...
				$(document).on("click", ".save", function() {
					...
					var tablelistin = [];
					$.each($(".prodin tbody tr"), function(i, item) {
					...
					postdata["tableListin"] = tablelistin;
					
					var tablelistout = [];
					$.each($(".prodout tbody tr"), function(i, item) {
						if($(item).find("input[name='goodsId']").val() == "") {
							//$(item).remove();
						} else {
							var tr = getinput2json(item);
							tablelistout.push(tr);
						}
					});

					postdata["tablelistout"] = tablelistout;
					
					postdata = $.mergeJsonObject(postdata, postdata1);
			console.log(postdata);
					var the_url = "hhdSave.do";
					$.ajax({
					type : "post",
					url : the_url,
					...
					success : function(data){
						...
						if(data.result == 1){
							alert("保存成功");
							window.location.href = "receive.do"
						}else{
							alert("保存失败");
						}
					},
				...
				$(this).parents("tbody").find("tr").removeClass("selecttr");
				$(this).parents("tr").addClass("selecttr");
				...
				$(document).on("click", ".saveImei", function() {
					...
					$.each($(".snList tbody tr"), function(i, item) {
						var imeistr = getinput2json($(item));
						imeilist.push(imeistr);
					});

					$(".inputboxtable tbody tr.selecttr").find("input[name='stockIMEI']").val(JSON.stringify(imeilist));
	...
	<body>
				...
				<!--<input type="hidden" name="flag" value="1"> -->
					<div class="container-fluid">
						<div class="row">
							...
							<div class="col-xs-3 ">
								<div class="input-group">
									<span class="input-group-addon input-group-title" id="basic-addon1"> 供货单位：</span>
									<input type="text" class="form-control" placeholder="Username" aria-describedby="basic-addon1" name="inDepartmentId">
								</div>
							</div>	
							<div class="col-xs-3">
								<div class="input-group">
									<span class="input-group-addon input-group-title" id="basic-addon1"> 换货员：</span>
		...
		<div class="grp_table">
			<table class="zxsaastable inputboxtable prodin">
				<thead>
					<tr>
						<th>行号</th>
						<th>操作</th>
				...
				入库：
					<tr>
						...
						<td><input type="text" name="taxPrice" /></td>
						<td><input type="text" name="taxAmount"/></td>
						<td><input type="text" name="discountAmount" /></td>
						<td><input type="text" name="discountedAmount"  /></td>
						<td><input type="text" name="taxRate" /></td>
						<td><input type="text" name="taxLimit"  /></td>
		...
		<div class="grp_table">
			<table class="zxsaastable inputboxtable prodout">
				<thead>
					<tr>
						<th>行号</th>
						<th>操作</th>
