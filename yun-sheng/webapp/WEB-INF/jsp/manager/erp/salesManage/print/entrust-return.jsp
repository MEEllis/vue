  ...
  <body style="padding: 10px;">
	...
	<c:if test="${tempKind == 'noimei'}">
		...
		<table class="table table-bordered" border-color="black">
			...
			<tbody>
				<c:forEach items="${bills.salesInstorExNumList}" var="billsDetail" varStatus="status">
					<tr style="padding: 0;">
				 ...
				 <tr style="padding: 0;">
					...
					<td style="font-size: 3mm;text-align: center;padding: 0;">${statistics:sumByField(bills.salesInstorExNumList, 'goodsNumber')}</td>
					<td style="font-size: 3mm;padding: 0;"></td>
					<td style="font-size: 3mm;text-align: right;padding: 0;"><fmt:formatNumber value="${statistics:sumByField(bills.salesInstorExNumList, 'amount')}" pattern="#,##0.00#"/></td>
		...
		<table class="table table-bordered" border-color="black">
			...
			<tbody>
				<c:forEach items="${bills.salesInstorExNumList}" var="billsDetail" varStatus="status">
					...
					<!-- 串号列表不为空，则输出 -->
					<c:if test="${(billsDetail.salesInstorExImDraftList)!= null && fn:length(billsDetail.salesInstorExImDraftList) > 0}">
					<tr style="padding: 0;">
						<td colspan="9" style="font-size: 3mm;text-align:left;height: auto;padding: 0;">
							<c:forEach items="${billsDetail.salesInstorExImDraftList}" var="detailImei">
							${detailImei.imei },
							</c:forEach>
						</td>
					</tr>
