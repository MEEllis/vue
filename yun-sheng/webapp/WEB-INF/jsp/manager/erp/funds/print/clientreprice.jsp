  ...
  <body style="padding: 10px;">
    ...
    <div style="font-size: 3mm;">
		...
		<div class="form-group" style="width: 62mm;float: left;margin-bottom: 1mm;">
		  ...
		  <span class="col-sm-8" style="padding-left: 3px;padding-right: 0px;">
		  <c:if test="${bills.subType!= null && bills.subType == '1'}">串号价保</c:if>
		  <c:if test="${bills.subType!= null && bills.subType == '2'}">数量价保</c:if>
		  </span>
		</div>
		<div class="form-group" style="width: 62mm;float: left;margin-bottom: 1mm;">
		  <label class="col-sm-4 control-label" style="padding-left: 1px;padding-right: 0px;width: 17mm;text-align: right;position: relative;float: left;">
	...
	<!-- 单据明细 -->
	<c:if test="${bills.subType!= null && bills.subType == '1'}">
	<table class="table table-bordered">
		<thead >
			<tr>
				...
				<th style="font-size: 3mm;font-weight: bold;text-align: center;">价保数量</th>
				<th style="font-size: 3mm;font-weight: bold;text-align: center;">价保总额</th>
				<th style="font-size: 3mm;font-weight: bold;text-align: center;">备注</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${bills.cashKhImsupNumList}" var="billsDetail" varStatus="status">
				<tr>
					...
					<td style="font-size: 3mm;text-align:center;">${billsDetail.surNum}</td>
					<td style="font-size: 3mm;text-align:right;"><fmt:formatNumber value="${billsDetail.surDiffAmount}" pattern="#,##0.00"/></td>
					<td style="font-size: 3mm;text-align:left;">${billsDetail.remark}</td>
				</tr>
				<!-- 串号列表不为空，则输出 -->
				<c:if test="${(billsDetail.cashKhImsupImList)!= null && fn:length(billsDetail.cashKhImsupImList) > 0}">
				<tr>
					<td colspan="6" style="font-size: 3mm;text-align:left;">
						${statistics:sumByField(billsDetail.cashKhImsupImList, 'imei')}
					</td>
				</tr>
				</c:if>
			</c:forEach>		
			<tr>
				...
				<td style="font-size: 3mm;height: auto;text-align: center;">
					${statistics:sumByField(bills.cashKhImsupNumList, 'surNum')}
				</td>
				<td colspan="2" style="font-size: 3mm;text-align: center;"><fmt:formatNumber value="${statistics:sumByField(bills.cashKhImsupNumList, 'surDiffAmount')}" pattern="#,##0.00"/></td>
			</tr>
		</tbody>
	</table>
	</c:if>
	
	<c:if test="${bills.subType!= null && bills.subType == '2'}">
	<table class="table table-bordered">
		<thead >
			<tr>
				<th style="font-size: 3mm;font-weight: bold;width: 13mm;text-align: center;">序号</th>
		...
		<tbody>
			<c:forEach items="${bills.cashKhSupnumList}" var="billsDetail" varStatus="status">
				<tr>
					<td style="font-size: 3mm;text-align:center;">${status.count}</td>
					<td style="font-size: 3mm;text-align:center;">${billsDetail.storageName}</td>
					<td style="font-size: 3mm;text-align:center;">${billsDetail.goodsName}</td>
					<td style="font-size: 3mm;text-align:right;"><fmt:formatNumber value="${billsDetail.surAmount}" pattern="#,##0.00"/></td>
					<td style="font-size: 3mm;text-align:left;">${billsDetail.remark}</td>
				</tr>
			</c:forEach>		
			<tr>
				<td colspan="3" align="center" style="font-size: 4mm;">合计</td>
				<td colspan="2" style="font-size: 3mm;text-align: center;"><fmt:formatNumber value="${statistics:sumByField(bills.cashKhSupnumList, 'surAmount')}" pattern="#,##0.00"/></td>
			</tr>
		</tbody>
	</table>
	</c:if>
