  ...
  <body style="padding: 10px;">
	...
	<c:if test="${printType == 'showimei'}">
		...
		<table class="table table-bordered" style="word-break: break-all; word-wrap:break-word;" bordercolor="black">
			...
			<tbody>
				<c:forEach items="${order.inDetailList}" var="billsDetail" varStatus="status">
					<tr style="padding: 0;">
					...
					<tr style="padding: 0;">
						<td colspan="9" style="font-size: 3mm;text-align:left;height: auto; padding:0;">
							<c:forEach items="${billsDetail.imeiList}" var="detailImei">
							${detailImei.imei },
							</c:forEach>
						</td>
					</tr>
				...
				<tr style="padding: 0;">
					...
					<td style="font-size: 3mm;text-align: center; padding:0;">${statistics:sumByField(order.inDetailList, 'goodsNumber')}</td>
					<td style="font-size: 3mm; padding:0;"></td>
					<td style="font-size: 3mm;text-align: right; padding:0;">${statistics:sumByField(order.inDetailList, 'amount')}</td>
					<td colspan="2" style="font-size: 3mm;text-align:center; padding:0;">${statistics:sumByField(order.inDetailList, 'discountedAmount')}</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="table table-bordered" style="word-break: break-all; word-wrap:break-word;" border-color="black">
			...
			<tbody>
				<c:forEach items="${order.outDetailList}" var="billsDetail" varStatus="status">
					<tr  style="padding: 0;">
						<td style="font-size: 3mm;text-align:center;padding: 0;">${status.count}</td>
						<td style="font-size: 3mm;text-align:left;padding: 0;">${billsDetail.storageName}</td>
						<td style="font-size: 3mm;text-align:left;padding: 0;">${billsDetail.goodsName}</td>
				...
				<tr style="padding: 0;">
					<td colspan="3" align="center" style="font-size: 4mm;padding: 0;">合计</td>
					<td style="font-size: 3mm;text-align: center;padding: 0;">${statistics:sumByField(order.outDetailList, 'goodsNumber')}</td>
					<td style="font-size: 3mm;padding: 0;"></td>
					<td style="font-size: 3mm;text-align: right;padding: 0;">${statistics:sumByField(order.outDetailList, 'amount')}</td>
					<td colspan="2" style="font-size: 3mm;text-align:center;padding: 0;">${statistics:sumByField(order.outDetailList, 'discountedAmount')}</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="table table-bordered" border-color="black">
			<thead>
				<tr style="padding: 0;">
				<th style="font-size: 3mm;font-weight: bold;width: 13mm;text-align: center; paddding:0;">序号</th>
				<th style="font-size: 3mm;font-weight: bold;text-align: center; paddding:0;">付款类别</th>
				<th style="font-size: 3mm;font-weight: bold;text-align: center; paddding:0;">账户名称</th>
				<th style="font-size: 3mm;font-weight: bold;text-align: center; paddding:0;">付款金额</th>
				</tr>
			</thead>
			<tbody>
			    <% int indexx = 1; %>
				<c:forEach items="${order.paymentReceivedOrderVo.detailList}" var="payreceiptDetail" varStatus="status">
					...
					<tr style="padding: 0;">
					<td style="font-size: 3mm;text-align:center; paddding:0;"><%=indexx++ %></td>
					<td style="font-size: 3mm;text-align:left; paddding:0;">${payreceiptDetail.accountTypeName}</td>
					<td style="font-size: 3mm;text-align:left; paddding:0;">${payreceiptDetail.accountName}</td>
					<td style="font-size: 3mm;text-align:right; paddding:0;"><fmt:formatNumber value="${payreceiptDetail.amount}" pattern="#,##0.00"/></td>
					</tr>
					</c:if>
				</c:forEach>
				<tr style="padding: 0;">
					<td colspan="3" align="center" style="font-size: 4mm; paddding:0;">合计</td>
				    <td style="font-size: 4mm;text-align:right; paddding:0;">
				    <fmt:formatNumber value="${statistics:sumByField(order.paymentReceivedOrderVo.detailList, 'amount')}" pattern="#,##0.00"/>
				    </td>
				</tr>
			</tbody>
		</table>
	    ...
	    <div id="formBottom" style="font-size: 3mm;">
			...
			--><div class="form-group" style="width: 40mm;float: left;">
			  ...
			  <c:if test="${isR }">
			  <fmt:formatNumber value="${((order.billsAmount*-1)>0?order.billsAmount:(order.billsAmount*-1)) * 1}" pattern="#,##0.00"/>
			  </c:if>
			  <c:if test="${!isR }">
			  <fmt:formatNumber value="${order.billsAmount>=0?order.billsAmount:(order.billsAmount*-1)}" pattern="#,##0.00"/>
			  </c:if> 
			  </span>
			</div>
			<div class="form-group" style="width: 40mm;float: left;">
			  ...
			  <span class="col-sm-8" style="padding-left: 3px;padding-right: 0px;width: 20mm;">
				  <c:if test="${isR }">
				  <fmt:formatNumber value="${( ((order.billsAmount*-1)>0?(order.billsAmount*-1):((order.billsAmount*-1)*-1) )-(statistics:sumByField2(order.paymentReceivedOrderVo.detailList, 'amount', 'Double')*-1) ) * -1}" pattern="#,##0.00"/>
				  </c:if>
				  <c:if test="${!isR }">
				  <fmt:formatNumber value="${(order.billsAmount>=0?order.billsAmount:(order.billsAmount*-1)) - statistics:sumByField2(order.paymentReceivedOrderVo.detailList, 'amount', 'Double')}" pattern="#,##0.00"/>
				  </c:if> 
			  </span>
			</div>
	...
	<c:if test="${printType == 'noimei'}">
		...
		<table class="table table-bordered" border-color="black">
			...
			<tbody>
				<c:forEach items="${order.inDetailList}" var="billsDetail" varStatus="status">
					<tr style="padding: 0;">
						<td style="font-size: 3mm;text-align:center; padding:0;">${status.count}</td>
						<td style="font-size: 3mm;text-align:center; padding:0;">${billsDetail.storageName}</td>
						<td style="font-size: 3mm;text-align:center; padding:0;">${billsDetail.goodsName}</td>
						<td style="font-size: 3mm;text-align:right; padding:0;">${billsDetail.goodsNumber}</td>
						<td style="font-size: 3mm;text-align:right; padding:0;"><fmt:formatNumber value="${billsDetail.price}" pattern="#,##0.00"/></td>
						<td style="font-size: 3mm;text-align:right; padding:0;"><fmt:formatNumber value="${billsDetail.amount}" pattern="#,##0.00"/></td>
						<td style="font-size: 3mm;text-align:right; padding:0;"><fmt:formatNumber value="${billsDetail.discountedAmount}" pattern="#,##0.00"/></td>
						<td style="font-size: 3mm;text-align:left; padding:0;">${billsDetail.remark}</td>
				...
				<tr style="padding: 0;">
					<td colspan="3" align="center" style="font-size: 4mm; padding:0;">合计</td>
					<td style="font-size: 3mm; padding:0;">${statistics:sumByField(order.inDetailList, 'goodsNumber')}</td>
					<td style="font-size: 3mm; padding:0;"></td>
					<td style="font-size: 3mm; padding:0;">${statistics:sumByField(order.inDetailList, 'amount')}</td>
					<td colspan="2" style="font-size: 3mm;text-align:center; padding:0;">${statistics:sumByField(order.inDetailList, 'discountedAmount')}</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="table table-bordered" border-color="black">
			...
			<tbody>
				...
				<tr style="padding: 0;">
					<td colspan="3" align="center" style="font-size: 4mm; padding:0;">合计</td>
					<td style="font-size: 3mm; padding:0;">${statistics:sumByField(order.outDetailList, 'goodsNumber')}</td>
					<td style="font-size: 3mm; padding:0;"></td>
					<td style="font-size: 3mm; padding:0;">${statistics:sumByField(order.outDetailList, 'amount')}</td>
					<td colspan="2" style="font-size: 3mm;text-align:center; padding:0;">${statistics:sumByField(order.outDetailList, 'discountedAmount')}</td>
				</tr>
			</tbody>
		</table>
