...
<html>
  ...
  <body style="padding: 10px;">
	...
	<div id="billsDIV" style="width: 190mm;position: relative;">
		...
		<!-- 单据明细 -->
		<table class="table table-bordered"  style="word-break: break-all; word-wrap:break-word;" border-coloer="black">
			<thead >
				<tr style="padding: 0;">
					...
					<c:if test="${isPrice == true}">
						<th style="font-size: 3mm;font-weight: bold;text-align: center; padding:0;">调拨单价</th>
						<th style="font-size: 3mm;font-weight: bold;text-align: center; padding:0;">调拨总价</th>
					</c:if>
					<th style="font-size: 3mm;font-weight: bold;text-align: center; padding:0;">商品备注</th>
				</tr>
			</thead>
			<tbody>
				<c:set var="sumNumber" value="0"></c:set>
				...
				<c:forEach items="${order.detailList}" var="detail" varStatus="status">
					<c:set var="sumNumber" value="${sumNumber + detail.goodsNumber}"></c:set>
					<c:set var="sumPrice" value="${sumPrice + detail.price}"></c:set>
					<c:set var="sumAmount" value="${sumAmount + detail.amount}"></c:set>
					<tr style="padding: 0;">
				...
				<tr style="padding: 0;">
					...
					<td style="font-size: 3mm; padding:0;" align="center">${sumNumber}</td>
					<c:if test="${isPrice == true}">
						<td style="font-size: 3mm; padding:0;" align="center"><fmt:formatNumber type="number" maxFractionDigits="2" value="${sumPrice}"/></td>
						<td style="font-size: 3mm; padding:0;" align="center"><fmt:formatNumber type="number" maxFractionDigits="2" value="${sumAmount}"/></td>
					</c:if>
