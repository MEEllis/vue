...
Page({
  ...
  data: {
    ...
    installmentBusinessName: '全部',
    installmentBusinessData: [],
    keyWord: '',
    timeActive: 2,
    startDate: '',
    endDate: '',
  ...
  onLoad: function(options) {
    ...
    this.getInstallmentfeesList();
    this.getInstallmentBusinessList();
    this.searchSubmit();
  },
  ...
  tapAdvanced: function() {
    ...
    wx.navigateTo({
      url: `/pages/common/installMentfee/installMentfee?route=${currentPage.route}&barTitle=分期业务战报-查询条件`,
    })
  },
  ...
  //获取运营商名称集合
  getInstallmentfeesList: function() {
    var that = this;
    const {
      menuCode
    } = this.data;
    serviceCom.getInstallmentfeesList({
      menuCode,
    }).then(installmentfeesData => {
      that.setData({
        installmentfeesData,
      });
    })
  ...
  //获取运营商单位
  getInstallmentBusinessList: function() {
    var that = this;
    const {
      menuCode
    } = this.data;
    serviceCom.getInstallmentBusinessList({
      menuCode,
    }).then(installmentBusinessData => {
      that.setData({
        installmentBusinessData,
      });
    })
  ...
  getDataList: function() {
    ...
    request(api.getInstallMentfeeData, postData).then(res => {
  ...
  getTotalVo: function() {
    var that = this;
    const postData = this.getSearchParam();
    request(api.getInstallMentfeeTotalVo, postData).then(res => {
      that.setData({
